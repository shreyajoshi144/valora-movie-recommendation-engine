...
# (type-hints compatibility for Python < 3.10)
...
def apply_popularity_penalty(score_dict: dict, tmdb_df: pd.DataFrame) -> dict:
    ...
    adjusted = score / log(1 + popularity)^2

    Exponent of 2.0 creates a meaningful spread:
      pop=5   → divisor ≈ 3.2   (niche)
      pop=100 → divisor ≈ 21.2  (popular)
      pop=500 → divisor ≈ 38.6  (blockbuster)
    This gives ~12x ranking advantage to niche vs blockbuster titles.
    """
    pop_map = tmdb_df.set_index("tmdb_id")["popularity"].to_dict()
    result = {}
    for movie_id, score in score_dict.items():
        pop = pop_map.get(movie_id, 1.0)
        result[movie_id] = score / (np.log1p(max(pop, 1e-3)) ** 2.0)
    return result
...
def _fetch_poster_by_id(tmdb_id: int) -> Optional[str]:
...
def _search_tmdb_by_title(title: str) -> Tuple[Optional[int], Optional[str]]:
    """
